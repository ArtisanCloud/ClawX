...
- 默认配置文件位置是 `~/.clawx/config.json`（可用 `CLAWX_CONFIG` 覆盖）。
...
```bash
# 查看 agent 列表
clawx config agent list

# 新增/更新 agent
clawx config agent add --id reviewer --profile claude

# 指定 workspace 与超时
clawx config agent add --id project-a --profile codex --workspace /workspace/project-a --timeout 900

# 新增后直接设为默认 agent
clawx config agent add --id project-b --profile codex --default

# 切换默认 agent
clawx config agent default project-a
```

默认行为：
- 如果 `--workspace` 不传，自动使用：
  - `main` -> `~/.clawx/workspaces/main`
  - 其他 agent（如 `reviewer`）-> `~/.clawx/workspaces/reviewer`
- 写入 agent 时会自动补齐 `runtime.allowedRoots`，保证 workspace 可执行。

历史兼容说明：
- 若你之前配置的是 `~/.clawx/workworkspace/...`，服务启动时会自动迁移到 `~/.clawx/workspaces/...` 并改写配置。
...
- 写入后需要重启服务生效。
- 可通过环境变量 `CLAWX_CONFIG_ADMIN_USERS` 限制哪些用户可执行配置指令。
...
## `/new` 与 Codex CLI 的关系
- `/new` 是 ClawX 的会话命令，只创建 ClawX 自己的 `session_id`。
...
2. 启动服务：`go run ./cmd/clawx`
3. 观察启动日志包含：`clawx service started with N runtime(s) ...`
4. 在命中不同绑定的频道/聊天发消息，确认分别由对应 Agent 响应。

## 当前限制
- `clawx config` 向导当前只引导默认 Agent（`main`）的基础配置。
