...
```bash
go run ./cmd/synapsex config set channels.telegram.enabled true
go run ./cmd/synapsex config set channels.telegram.mode webhook
go run ./cmd/synapsex config set channels.telegram.webhookUrl https://<domain>/webhooks/telegram
...
- 若只想改 Telegram，不要执行全量 `synapsex config` 覆盖流程。
