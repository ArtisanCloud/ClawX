...
## 阶段交付摘要（2026-03-09）
- 交付状态：功能实现完成，进入发布门禁校验阶段。
- 已完成能力：
  - 窗口优先路由（`window_id -> current_session_id`）与 compat 回退路径。
  - 控制命令窗口语义（`/new`、`/resume`、`/switch`、`/list`、`/current`、`/cancel`）。
  - 最近使用时间刷新规则（new/resume/switch/execute_success）。
  - 多窗口隔离、同窗口切换、命令优先级、串行执行约束、字段持久化等回归覆盖。
- 回归结果：
  - `GOCACHE=$(pwd)/.gocache GOMODCACHE=$(pwd)/.gomodcache go test ./...` 已通过。

## 风险状态回写（2026-03-09）
- 风险 1（窗口概念与渠道概念混淆）：状态 `受控`
  - 处理进展：契约已明确“渠道仅归一化，不持有窗口绑定状态”。
- 风险 2（旧行为回退不兼容）：状态 `受控`
  - 处理进展：缺省窗口统一落 `compat:<conversation_id>`，兼容回退回归已覆盖。
- 风险 3（切换与恢复语义混淆）：状态 `关闭`
  - 处理进展：`/resume` 与 `/switch` 已拆分，契约与测试一致。
- 风险 4（最近会话排序失真）：状态 `受控`
  - 处理进展：在 new/resume/switch/execute_success 后刷新 `last_used_at`，相关测试通过。

## 发布门禁（SC 指标与人工验收）
- 数据窗口：最近 7 天
- 样本下限：每项 `>= 200`
- 指标来源：
  - 自动采集：`tests/integration/multi_session_metrics_report_test.go`
  - 人工验收：`docs/guides/phase_2/phase_2_validation.md`

当前门禁结论（2026-03-09）：
- `SC-001 ~ SC-004`：`BLOCK`
  - 原因：采集脚本已就绪，但尚未注入真实 7 天指标数据。
- `SC-005`：`BLOCK`
  - 当前通过率：100%（20/20 预验收样本）
  - 阻塞原因：样本量未达到 200。
...
- 数据门禁：`BLOCK`
- 进入正式发布前置动作：
  - 补齐 `SC-001 ~ SC-004` 的真实指标采集并生成报告。
  - 扩充 `SC-005` 人工验收样本至不少于 200。
