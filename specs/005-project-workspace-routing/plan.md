...
**结构决策**: 延续单服务 Go + DDD；项目相关状态先用文件存储，不引入数据库。`/project` 命令与自动建议均复用同一应用服务，避免双逻辑分叉。`intent/pipeline` 仅负责候选项目识别与建议触发；proposal 持久化、确认生效、绑定更新等状态写入统一由 `internal/application/project` 负责。
