...
**当前状态说明**: `Phase 1` 到 `Phase 6` 的已完成项表示“骨架与代码路径已落地”，不等于“第一阶段 MVP 已具备真实端到端可用性”。真实服务化与渠道联调补完见本文件末尾的 `Phase 7`。
...
**限制说明**: 当前验证仍是骨架级验证，尚未形成真实渠道 SDK 接入与真实后端执行。
...
## Phase 7：MVP 补完（阻塞 Phase 2 开发）

**目的**: 把第一阶段从“可编译骨架”补完为“可配置渠道、可真实执行、可常驻运行”的最小可用版本
...
- [X] T051 将统一入站消息真正接入 Router，使渠道消息可以驱动“新建 / 恢复 / 继续 / 控制命令”主链路于 internal/interfaces/chat/normalize.go 和 internal/application/service/
- [X] T052 将占位式 DirectRunner 替换为一个真实执行器，至少完成一次真实命令调用与结果回传于 internal/infrastructure/backend/runner.go
- [X] T053 补齐真实后端执行所需的运行边界、超时、取消和错误映射，确保真实执行路径与现有会话锁语义一致于 internal/infrastructure/backend/
- [X] T054 补充最小端到端集成测试，覆盖“渠道入站 -> 会话路由 -> 真实执行 -> 出站回写”于 tests/integration/phase1_foundation_test.go
- [X] T055 重写第一阶段运行与验证说明，明确真实配置、启动命令与人工验收步骤于 docs/guides/phase_1/phase_1_mvp_guide.md 和 specs/001-phase1-foundation/quickstart.md
- [X] T056 完成一次最小人工验收，并回写 Phase 1 实际可用状态与未完成项于 docs/plans/phase_1_foundation.md

**检查点**: 完成本阶段后，才可认定第一阶段已达到“小 MVP 能跑通”的标准
...
- **Phase 7（MVP 补完）**: 依赖 Phase 1-6 完成；阻塞 Phase 2 的代码开发
...
6. 在进入下一阶段前，完成 Phase 7 的真实运行补完与人工验收
...
- 第二阶段的代码开发以 Phase 7 完成为前提；否则仅可继续补完第一阶段的可用性
